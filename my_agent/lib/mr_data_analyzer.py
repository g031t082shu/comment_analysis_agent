...
#         tokens_count = model.count_tokens(text)

#         return  tokens_count.total_tokens

#     def _calc_data_per_chank(self, data: list) ->int:
#         # 全コメントのトークン数を計算
#         tokens = self._calc_tokens(f'{data}')

#         if tokens < _model_info.input_token_limit:
#             return len(data)
#         else:
#             # １コメントあたりの平均トークン数を計算
...
#             max_token = int(int(_model_info.input_token_limit) * 0.8)
#             data_num_per_chunk = int(max_token / tokens_per_data)
            
#             return data_num_per_chunk

#     def _invoke_chain(self, user_inpuut: str, data: list) -> BaseMessage:
...
#     def _summarize_chank(self, user_input: str, texts) -> BaseMessage:
#         prompt = ChatPromptTemplate.from_messages([
#             ("system", "あなたの仕事は、与えられたテキストのリストを要約し適切な結果を出力することです。"
#                        "あなたには以下の形式でテキストのリストが与えられます。"
#                        "[text1, text2, text3, ...]"
#                        "上記のリストは全て以下の質問に対するllmの出力です。"
#                        "質問：{question}"                    
#                        "ただし、出力結果は上記の質問の要求を満たす必要があることに注意してください。"),
#             ("user", "{texts}")
#         ])
...
#         data_per_chank = self._calc_data_per_chank(data)

#         for i in range(0, len(data), data_per_chank):
#             res = self._invoke_chain(input, data[i: i+data_per_chank])
#             responses.append(res.content)

#         summary = self._summarize_chank(input, responses)
    ...
    def _summarize_chank(self, user_input: str, texts,) -> BaseMessage:
        prompt = ChatPromptTemplate.from_messages([
            ("system", "あなたの仕事は、与えられたテキストのリストを要約し適切な結果を出力することです。"
                       ...
                       "上記のリストは全て以下の質問に対するllmの出力です。"
                       "質問：{question}"
                       "なお、出力結果は上記の質問の要求を満たす必要があることに注意してください。"),
            ("user", "{texts}")
        ])
